      ...
      dbTransaction.beginTransaction((err, transaction) => {
        transaction.run(
          ...
          (err) => {
            ...
            var placeholders = arrayStudents
              .map(() => {
                return schedule
                  .map(
                    (id) =>
                      `(?, ?, (SELECT id FROM journal WHERE schedule_id =  ${id} AND date_pair = "${req.body.date_pair}"))`
                  )
                  .join(",");
              })
              .join(",");
            transaction.run(
              `INSERT INTO journal_student(student_id, attendance, journal_id) VALUES ` +
                placeholders,
              journal_student,
              (err) => {
                if (err) {
...
router.post("/getDataForAddingAttendance", function (req, res) {
  ...
  var student_group_id = req.body.student_group_id;
  ...
  db.all(
    ...
    (err, rows) => {
      ...
      db.all(
        `SELECT * FROM student WHERE student.student_group_id=?`,
...
router.post("/deleteJournal/:id", (req, res) => {
  ...
  db.run("DELETE FROM journal WHERE schedule_id=?", [req.params.id], (err) => {
    if (err) {
      throw err;
    }
  });
